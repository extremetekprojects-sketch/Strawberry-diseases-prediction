...
async def detect_image(file: UploadFile = File(...)):
    try:
        ...
        for r in results:
            ...
            if boxes is not None:
                for box in boxes:
                    # box.cls and box.conf and box.xyxy should be accessible
                    cls_idx = int(getattr(box, "cls", 0))
                    detections.append({
                        "class": r.names.get(cls_idx, str(cls_idx)),
                        "confidence": float(getattr(box, "conf", 0.0)),
                        "bbox": getattr(box, "xyxy", []).tolist()[0] if hasattr(box, "xyxy") else []
                    })
...
async def detect_video(file: UploadFile = File(...)):
    try:
        ...
        return {
            "message": "Video processed successfully",
            "output_video_path": output_video_path,
            "total_frames": len(results)  # results usually iterable per frame
        }
    except Exception as e:
        return {"error": f"Video detection failed: {str(e)}"}

# NEW: Serve processed video using FileResponse
@app.get("/video/{video_path:path}")
async def get_video(video_path: str):
    """Serve processed video file for streaming/download."""
    try:
        ...
        if not os.path.exists(full_path):
            return JSONResponse({"error": f"Video not found: {full_path}"}, status_code=404)
        
        # Using FileResponse ensures correct headers and streaming behavior
        return FileResponse(full_path, media_type=mimetypes.guess_type(full_path)[0] or "video/avi", filename=os.path.basename(full_path))
    except Exception as e:
        return JSONResponse({"error": f"Failed to serve video: {str(e)}"}, status_code=500)
